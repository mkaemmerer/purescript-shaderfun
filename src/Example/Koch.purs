...
import Graphics.DomainTransform (mirror, mirrorY, translate)
...
kochFold n = (repeatC n (kFold >>> setup)) >>> inverseSetup
  where
  kFold = mirror (tau / 6.0) >>> translate (Vec2 {x: 0.5, y: 0.0}) >>> mirrorY
...
program = shaderProgram $ fractal >=> signRamp
  where
    baseFractal = kochFold 5 $ segment (Vec2 {x: 0.0, y: 0.0}) (Vec2 {x: 1.0, y: 0.0})
